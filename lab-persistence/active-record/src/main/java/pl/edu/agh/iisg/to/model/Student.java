...
//String enroll = "INSERT INTO student_course(..) VALUES (?,?)"
//QueryExecutor.createAndObtainId(enrollStudentSQL, student.id(), id());
//return true;
...
public class Student {
    ...
    public static Optional<Student> findByIndexNumber(final int indexNumber) {
        ...
        try {
            ...
            String firstName = rs.getString(Columns.FIRST_NAME);
            String lastName = rs.getString(Columns.LAST_NAME);
            int indexNum = rs.getInt(Columns.INDEX_NUMBER);

            return Optional.of(new Student(id, firstName, lastName, indexNum));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return Optional.empty();
